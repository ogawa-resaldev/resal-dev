  ...
  module Helpers
    module ApplicantHelper
      ...
      def createApplicant(params)
        ...
        # kiwami用
        if params["experience-num"].present? then
        ...
        response.each do |res|
          if params["text-shop"] == res["store_name"] then
        ...
        # 店舗別のパラメータ
        if params["text-percentage"].present? then
          self_pr = self_pr + "\r\n\r\n【体脂肪率】\r\n" + params["text-percentage"]
        end
        if params["spec-type"].present? then
          self_pr = self_pr + "\r\n\r\n【強み区分】\r\n" + params["spec-type"].join("、")
        end
        if params["spec-exp"].present? then
          self_pr = self_pr + "\r\n\r\n【強み詳細】\r\n" + params["spec-exp"]
        end
        if params["experience-exp"].present? then
        ...
        # メールが検索できるまでに時間差があるかもしれないので10秒待つ。
        sleep(10)

        # 画像を取得して追加する。
        images = getLatestImages(applicant)
        case images.length
        when 0
          # 何もしない
        when 1
      ...
      def getLatestImagesFromFront(applicantId)
        images = getLatestImages(Applicant.find(params[:applicantId])).map do |image|
          {
      ...
      private def getLatestImages(applicant)
        ...
        params = {
          ...
          query: applicant.applicant_detail.mail_address,
          start: start,
          max: max
        }

        res = HTTParty.post(
          Store.find(applicant.applicant_store_id).store_group.mail_api,
