class ApplicantLineTemplate < ApplicationRecord
  acts_as_paranoid
  belongs_to :store_group
  validates :line_template_name,
  ...
  scope :all_by_user, -> (user_id){
    ...
    # 管理者
    when 2 then
      ApplicantLineTemplate.all
    # 内勤
    when 3 then
      store_group_id = BackOfficeGroup.find_by(user_id: user_id)[:store_group_id]
      ApplicantLineTemplate.where(store_group_id: store_group_id)
    end
  }
end
