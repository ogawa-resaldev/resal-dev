class ApplicantsController < ApplicationController
  ...
  def index
    @applicants = Applicant.all_by_user(session[:user_id]).joins(:applicant_detail).order(:pending_flag, 'applicant_details.application_date DESC')
    # 店舗グループによる絞り込み。
    if params[:store_group_id].present? then
      store_ids = Store.where(store_group_id: params[:store_group_id]).pluck(:id)
      @applicants = @applicants.where(applicant_details: { preferred_store_id: store_ids })
    end
    # ステータスによる絞り込み。
  ...
  def update
    ...
    begin
      ...
      if params[:send_line_flag] == "1" then
        ...
        send_line(push_target_id, store_group, params[:line_body])
      end
